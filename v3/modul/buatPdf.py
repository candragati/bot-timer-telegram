...
def buatPdf(chat_id):    
    ...
    for i in range(jumR):
        ...
        mc = str(barR[i][3])                
        if mc=='':
          m_chat = Paragraph(str(message_chat).replace("<","&lt;").replace(">","&gt;").replace("\\n","<br/>").replace("\\u","&#x"), styles["Line_Label"])
        else:   
          m_chat = Image("gambar/%s"%mc,width = 128, height = 128)
        
        data1 = [[Paragraph('', styles["Line_Label"]),m_chat,Paragraph('', styles["Line_Label"]),]]
        dataLISTwidth = ('8%','8%','84%')
        t1 = Table(data1, colWidths=dataLISTwidth)
        t1.setStyle(TableStyle([
          # ('INNERGRID', (0, 0), (-1, -1), 0.25, colors.black),
...
# buatPdf(-1001337729941) # koding Teh
